<x-app-layout>
    <div class="relative z-10">
        <!-- Title -->
        <div class="max-w-2xl mb-5 {{$myeventsfinished->isEmpty() ? 'hidden' : ''}}">
            <h2 class="text-2xl font-bold md:text-2xl md:leading-tight dark:text-white">Semua Acara Selesai</h2>
        ...
        <!-- Grid -->
        <div class="grid {{$myeventsfinished->isEmpty() ? 'lg:grid-cols-1' : 'lg:grid-cols-2'}} lg:gap-y-16 gap-10">
            <!-- Card -->
            ...
            @else
            @foreach ($myeventsfinished as $events )
            <div
                    ...
                    <div class="absolute right-2 top-2 z-10">
                        <div
                            ...
                            <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden z-10 mt-2 min-w-40 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-lg p-2 dark:bg-neutral-800 dark:border dark:border-neutral-700 dark:divide-neutral-700"
                                ...
                                @php
                                $oneDayAfterEnd = strtotime($events->end_date . ' +1 day');
                                @endphp

                                @if(time() < $oneDayAfterEnd) <a
                                    class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300 dark:focus:bg-neutral-700 mb-1"
